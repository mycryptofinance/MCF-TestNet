...
const CONTRACT_CONFIG = {
    ...
    46630: {
        ...
        nativeTicker: "ETH",
        // НОВЫЙ АДРЕС КОНТРАКТА MCF ДЛЯ Robinhood Chain Testnet
        mcf: "0x1B38eFfeEbc9525E6FD129E9fE64038781f23346",
        nft: "0xB499CDDDdF296e247cCCABAB530cD21a8c36a0aA",
        staking: "0x8eC0aDCe1C8CC5EA3Ed7c725A61e826F5c608C52",
...
async function connect() {
    ...
    try {
        ...
        if (typeof renderUserTokens === "function") {
            await renderUserTokens(userAccount, currentChainId);
        }
        // ---------------------------------------
...
async function fetchMCFData() {
    ...
    try {
        ...
        // Форматируем числа для текста "2 047 110 / 1 000 000 000 SOLD"
        const formattedTotal = parseFloat(ethers.utils.formatEther(total)).toLocaleString('ru-RU');
        const formattedMax = parseFloat(ethers.utils.formatEther(max)).toLocaleString('ru-RU');
        document.getElementById('progressText').innerText = `${formattedTotal} / ${formattedMax} SOLD`;

        // 2. ВЫЗЫВАЕМ ОБНОВЛЕНИЕ ШКАЛЫ
        // Передаем значения, предварительно конвертировав их из BigNumber
        updateProgressBarUI(
            ethers.utils.formatEther(total), 
            ethers.utils.formatEther(max)
        );
...
function updateProgressBarUI(sold, max) {
    // Используй тот ID, который у тебя в HTML (supplyBar или progressBarFill)
    const bar = document.getElementById('supplyBar'); 
    
    if (!bar || !max || max == 0) return;

    // Считаем процент (sold и max уже обычные числа после formatEther)
    const percentage = (sold / max) * 100;
    // Ограничиваем от 0.1% до 100%, чтобы даже при нуле была видна тонкая полоска
    const safePercent = Math.min(Math.max(percentage, 0.1), 100).toFixed(2);

    // Применяем ширину
    bar.style.width = safePercent + '%';
    
    // Если у тебя есть элемент для вывода цифр процента (например 0.25% SOLD)
    const percentLabel = document.getElementById('soldPercentage');
    if (percentLabel) {
        percentLabel.innerText = `${safePercent}% SOLD`;
    }
}
    ...
    for(let i=1; i<=8; i++) {
        try {
            ...
            if(done) { 
                ...
                if(btn.parentElement && btn.parentElement.classList.contains('')) {
...
const addRobinhood = async () => {
    try {
        await window.ethereum.request({
            method: 'wallet_addEthereumChain',
            params: [{
                chainId: '0xB626', // Это 46630 в hex формате
        ...
        console.error("Ошибка при добавлении сети:", e);
        return false; 
    }
};
...
async function switchNetwork(chainId) {
    ...
    if (chainId === 46630) await addRobinhood();
...
let particles = [];
// Оптимальное количество: 60-70 не грузит мобильные устройства
...
class Particle {
    constructor() {
        ...
        // СКОРОСТЬ: увеличена до 0.8 - 1.2 (было 0.3)
    ...
    draw() {
        ...
        // Используем статический цвет без теней для скорости
...
function animate() {
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
            ...
            const dx = p1.x - p2.x;
            const dy = p1.y - p2.y;
            const distSq = dx * dx + dy * dy; // Считаем квадрат расстояния (быстрее, чем корень)
